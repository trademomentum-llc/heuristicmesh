...
|----|----------|------|-------|
| E-1 | 🔴 | **Scientific-claims alignment** | No repository source currently substantiates the quoted "muscular contractions ... CNS activity" premise. The current hardware (AMG8833 8×8 at ~20 Hz in unified firmware; MLX90640 32×24 burst at 8 Hz) supports gross thermal distribution and motion-derived features, but not direct resolution of muscular contractions or CNS activity; cite the source and re-scope outward claims, consent language, and the 510(k)/MDR narrative to "thermal motion-pattern analysis" or drop the claim. |
...
| E-4 | 🟡 | Framework 3 implementation and confidence-calibration methodology absent | `jetson/hm_ingest_unified.py` contains Framework 2 heuristics only; no Framework 3 classifier or output is implemented. Thresholds are placeholders pending baseline data (E-3 dependency). |
